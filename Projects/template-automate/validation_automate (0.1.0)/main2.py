...
def template_list() -> list[str]:
    ...
    return [os.path.join(resource_path("Templates"), file) for file in os.listdir(resource_path("Templates"))]

# Crear el directorio para los documentos renderizados
verify_path(path=resource_path("rendered_templates"))

# Abrir el archivo de Excel y obtener la hoja de cálculo
workbook = load_workbook(resource_path("data_base.xlsx"))
...
for row in panel_ws.iter_rows(min_row=3, values_only=True):
    ...
    verify_path(os.path.join(resource_path("rendered_templates"), str(context[keys[0]])))
    
    # Iterar sobre cada plantilla y renderizarla
    for template_path in template_list():
        # Construir la ruta de salida del documento renderizado
        output_path = resource_path(os.path.join("rendered_templates", str(context[keys[0]]), f"rendered_{os.path.basename(template_path)}"))
